...
//hotkey
$(".command-line").keydown(function (event){
	if(event.keyCode==38){
		//up key
		localStorage.currentCmd=$(".command-line").val();
		$(".command-line").val(localStorage.previousCmd);
	}else if(event.keyCode==40 && localStorage.currentCmd){
		//down key
		$(".command-line").val(localStorage.currentCmd);
	}
});

$(".command-line").focus();
$('.command-line').focusout(function(){
	if($(this).data("always-focus")==1){
		$('.command-line').focus();
	}
});
//process
